...
4. **UI restructure** — split into 3 sub-cycles. Final tab layout is **Physical Tables / Rounds / Results / BOS** (Physical Tables kept as a 4th tab; resolved 2026-05-24).
   ...
   - ⏳ **6c: Migrate MedalRoundView + delete old Tables + Medal Rounds tabs + fix broken tests.** Final tab order: Physical Tables / Rounds / Results / BOS (indices 0–3). Also: drop V22 columns (medal_round_status, physical_table_id, medal_round_mode rename → mode); delete unused service methods (startMedalRound / completeMedalRound / reopenMedalRound / resetMedalRound / assignMedalRoundToPhysicalTable).
