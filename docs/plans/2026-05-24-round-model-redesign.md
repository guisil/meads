...
4. **UI restructure** — split into 3 sub-cycles. Final tab layout is **Physical Tables / Rounds / Results / BOS** (Physical Tables kept as a 4th tab; resolved 2026-05-24).
   ...
   **Deferred (post-walkthrough cleanup):** MedalRoundView still reads from `CategoryJudgingConfig.medalRoundStatus` rather than the medal `JudgingRound.status` — the cascade keeps the JudgingRound in sync but it's not the read-of-record yet. Blocks: V22 column contraction (drop `medal_round_status`, `physical_table_id`, rename `medal_round_mode → mode`) + service-method deletion (`startMedalRound`/`completeMedalRound`/`reopenMedalRound`/`resetMedalRound`/`assignMedalRoundToPhysicalTable`). All of this is non-user-visible plumbing — current state is correct, just transitional.
