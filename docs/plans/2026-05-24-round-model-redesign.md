...
3. **Model + service changes — partially done.**
   ...
   - ✅ Cascade migration (`cascadeMarkCategoryReadyIfAllTablesComplete`) marks any PENDING medal `JudgingRound` READY when all SCORING rounds in the category COMPLETE. Filters to SCORING-only rounds for the "all complete" check. Legacy `CategoryJudgingConfig.medalRoundStatus` still updated as a transitional bridge until UI migrates (cycle #4). SCORE_BASED auto-fill via `MedalAward.confirmed=false` not yet wired here — still on `startMedalRound`. Done 2026-05-24.
...
4. **UI restructure** — split into 3 sub-cycles. Final tab layout is **Physical Tables / Rounds / Results / BOS** (Physical Tables kept as a 4th tab; resolved 2026-05-24).
   - ✅ **6a: Add Rounds tab.** Done 2026-05-24. Tab appended at index 4 (after BOS) so existing tab-index tests stay valid; cycle 6c will renumber. Type filter ComboBox (All/Scoring/Medal) + 8-column grid (Type/Name/Category/Physical Table/Status/Judges/Scheduled/Actions). Open button drills into `RoundView` (SCORING) or `MedalRoundView` (MEDAL). i18n in 5 locales. +1 view test (`shouldRenderRoundsGridAndTypeFilterOnRoundsTab`); +1 net test (1161 → 1162).
   - ✅ **6b: Add Results tab.** Done 2026-05-24. Tab appended at index 5. `results-grid` filters to COMPLETE rounds; 6 columns (Type/Name/Category/Physical Table/Outcome/Actions). Outcome formatting is type-aware: SCORING shows "{n} scoresheets submitted"; MEDAL shows G:n S:n B:n W:n. Empty-state caption "No completed rounds yet." i18n in 5 locales. +1 view test (`shouldRenderResultsGridShowingCompleteRoundsOnly`); +1 net test (1162 → 1163).
   - ✅ **6c: Delete old Tables + Medal Rounds tabs + fix broken tests.** Done 2026-05-24. Final tab order: Physical Tables (0) / Rounds (1) / Results (2) / BOS (3). Add Round button + per-row SCORING actions (Edit/Start/Assign-Judges/Delete/Open) moved to Rounds tab; MEDAL rows get just an Open button (full management in MedalRoundView). New Add Round dialog has a Type selector that swaps between SCORING (creates a scoring round via `createRound`) and MEDAL (creates via `createMedalRound`). 5 obsolete tests deleted (coverage retained in `JudgingServiceMedalRoundTest` + `MedalRoundViewTest`); tab indices renumbered. New i18n keys (`judging-admin.rounds.add`, `judging-admin.rounds.added`) in EN/PT/ES/IT/PL. 1163 → 1158 net.

   **Deferred (post-walkthrough cleanup):** ~~MedalRoundView still reads from `CategoryJudgingConfig.medalRoundStatus`~~ ✅ **DONE 2026-05-24 (cycle 7).** MedalRoundView (+ JudgingAdminView, RoundView, ScoresheetServiceImpl, JudgingServiceImpl guards) now read from medal JudgingRound via `getEffectiveMedalRoundStatus`. Writes go through new round-id methods `completeMedalRoundById`/`reopenMedalRoundById`/`resetMedalRoundById`. V22 + V29 contracted (dropped `medal_round_status` + `category_judging_configs.physical_table_id`; `medal_round_mode` rename to `mode` skipped — cosmetic). Legacy `startMedalRound`/`completeMedalRound`/`reopenMedalRound`/`resetMedalRound`/`assignMedalRoundToPhysicalTable` deleted along with `CategoryJudgingConfig.medalRoundStatus`/`physicalTableId` fields + all CJC state-machine methods + `MedalRoundStatus` enum + `autoPopulateMedalsByScore` helper. 1158 → 1154 tests (-4 obsolete-test deletion). **New gap:** no Start UI for medal rounds — see "Open gap" section above. Auto-fill on SCORE_BASED needs to re-emerge when the Start button arrives.
