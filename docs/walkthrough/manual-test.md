...
- [ ] **Assign Final Category on an entry:** go to Entries tab on Entry Admin → edit a SUBMITTED entry → **Expected:** the primary **Category** dropdown lists each subcategory exactly once (no duplicates from the cloned JUDGING-scope rows). Final Category dropdown lists JUDGING-scope categories (clearable); pick one, Save; entry's Final Category column updates from "—" to the picked code
