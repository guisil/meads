...
### CURRENT (2026-05-26, evening): Wrapped for the day — Option A + 8 follow-ups landed

The walkthrough paused mid-§12.6.8 to ship **Option A** (SCORE_BASED medal
rounds can own scoresheets — the small-category MEDAL — SCORING use case),
followed by **eight follow-up commits** as the user exercised the new flow
manually. See the **Tests:** line above for the full per-commit summary.

**State on disk** (`feature/judging-module` at `e260aa2`, 1225 tests passing,
pushed to origin). Verify with `mvn test -Dsurefire.useFile=false`. Top
walkthrough checkpoint was still mid-§12.6.8 when the day ended.

**BEFORE resuming the walkthrough — two design questions the user raised at
end-of-day:**

1. **Should SCORE_BASED medal rounds force all RECEIVED entries in the
   judging category to be assigned (no partial assignment allowed)?** Today
   the Assign Entries dialog on MedalRoundView (`openAssignEntriesDialog`)
   accepts any subset. For the no-prelim flow that may be wrong — every
   RECEIVED entry in the category arguably MUST go to the only medal venue
   that exists. Tradeoff: forcing-all is simpler and matches the
   "every entry must be judged" intuition; allowing partial preserves admin
   discretion (e.g., entry not bottled in time, withdrawn-but-not-marked).
   Decide before resuming. If we force-all, the dialog probably becomes a
   read-only "X RECEIVED entries will be assigned" preview + Save, and
   subsequent RECEIVED entries auto-assign via a new listener on entry
   status changes. (COMPARATIVE keeps the existing flow — admin picks
   advance-flagged subset.)

2. **Auto-populate tie-resolution: cover the "non-tied entries past a tied
   slot still resolve" case.** Today `autoPopulateMedalsByScore` walks
   gold→silver→bronze and **stops on the first tie within a slot** — no
   medals are written at all. User wants smarter behavior: if scores are
   90 / 90 / 70, gold stays unresolved (the two 90s) but the 70 should
   still auto-resolve to bronze (or whatever the appropriate medal is)
   without waiting for the admin to break the gold tie. Also test the
   no-ties case (e.g., 95 / 88 / 80 → all three auto-medaled at start /
   on listener firing). Both cases probably want a new flow:
   "auto-fill non-tied slots; flag tied slots as unresolved."
   Interacts with the medal-cap rule (e260aa2 — 1 each G/S/B): when admin
   resolves the tie, they pick which entry gets which medal, and the cap
   enforces it. Consider that two of the three entries already have
   medals at the time of tie-resolution (gold-tie left unresolved, silver
   auto-awarded to the next score, bronze auto-awarded). Probably the
   admin-tie-resolution step needs to support reshuffling — e.g.
   demoting the auto-silver to bronze to free silver for one of the tied
   gold candidates. Best to walk through a few scenarios before coding.

**Walkthrough resume checklist (after the two design questions are settled):**
