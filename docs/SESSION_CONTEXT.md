...
**Tests:** 1115 passing (`mvn test -Dsurefire.useFile=false`) — verified 2026-05-23 after the `DivisionAdvanceGuard` family, `AwardsService.republish` refactor, BOS-places lock moving from REGISTRATION_CLOSED to JUDGING, and the deadline-stale-value Settings-tab bug fix (+17 tests vs. the 1098 audit-fixes baseline).
