...
- **P16 (clarify the RoundView Total `*` suffix): DEFERRED into the scoresheet redesign** (raised 2026-05-30). The Total column shows a running/provisional sum with a trailing ` *` for FILLED/DRAFT (not-yet-SUBMITTED) sheets vs. the locked total (no `*`) for SUBMITTED sheets (`RoundView.formatTotalCell`). It's working as intended but not self-explanatory — a user had to ask. Add a Total-column-header tooltip or a small legend caption (e.g. "`*` = provisional total; locks when the round is finalized") when reworking the scoresheet UI.
