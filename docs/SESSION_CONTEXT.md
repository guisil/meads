...
**Tests:** 1107 passing (`mvn test -Dsurefire.useFile=false`) — verified 2026-05-23 after the `DivisionAdvanceGuard` family (now including `EntryFinalCategoryAdvanceGuard` blocking REGISTRATION_CLOSED → JUDGING with unassigned SUBMITTED/RECEIVED entries), `AwardsService.republish` refactor, BOS-places lock moving from REGISTRATION_CLOSED to JUDGING, the deadline-stale-value Settings-tab bug fix, the removal of the per-competition comment-languages restriction (judges now pick from all ISO 639-1 codes), and the dev seed beef-up (Amadora 10 entries with status variety + Profissional 20 entries pre-staged at JUDGING).
