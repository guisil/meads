...
anonymization, change #36) is **done locally but NOT yet committed**. **1301 tests green on JDK 25.**
**NEXT:** (1) review + commit/push change #36; (2) **P18 part 2 — the scoresheet field-layout redesign**
(awaiting the user's screenshots/examples) + fold in **P15** (bottom-notification overlap); (3) the still-
unrun **Awards walkthrough §13.6 onward** (freeze guard → revert → re-publish → announcements → anonymity);
(4) then code review → merge to main → **v0.4.0 release**. The earlier (2026-05-31) resume pointer follows
for reference:
