...
1. ~~**Allow judging management at REGISTRATION_CLOSED** (task #23)~~ — **DONE 2026-05-25**. Three changes: `JudgingAdminView.beforeEnter` gate lowered from `< JUDGING` to `< REGISTRATION_CLOSED`; `DivisionDetailView` "Manage Judging" button now visible from REGISTRATION_CLOSED; service-level gate on `JudgingService.startRound` requiring `>= JUDGING` (key `error.round.cannot-start-before-judging` × 5 locales). Walkthrough §12.4.2 / §12.5.1 / §12.6 / §12.6.4 updated. 1179 tests passing (+2).

2. ~~**Cross-division shared tables flag** (task #21)~~ — **DONE 2026-05-25**. `Competition.sharedTables` boolean (default TRUE), V30 migration. `JudgingService.startRound` cross-division busy-check by label when flag ON (key `error.round.physical-table-busy-shared` × 5 locales). UI: Settings checkbox + Physical Tables tab banner. Three new tests + judge active-conflict cross-division regression test. 1182 tests passing (+3).

3. ~~**Medal-round redesign** (task #24)~~ — **DONE 2026-05-25**. V31 partial unique index backstops one-medal-round-per-category; cascade populates `round.entries` per mode (COMPARATIVE → advance flag; SCORE_BASED → all submitted); `findMedalRoundEntries` reads from `round.entries`; `MedalRoundView` has an Assign Entries dialog mirroring the Rounds-tab equivalent. 8 new i18n keys × 5 locales. Two new tests + the existing suite unchanged via fallback. 1189 tests passing (+4 across the three commits).

4. ~~**Scoresheet improvements** (task #25)~~ — **DONE 2026-05-25**. Three sub-items shipped: per-item comment TextArea per MJP field; comment language fallback to `User.preferredLanguage`; `meadName` hidden from judges (admins still see it). New i18n key `scoresheet.scores.comment.placeholder` × 5 locales. Three new tests. Walkthrough §12.11 refreshed. 1185 tests passing (+3).

5. ~~**i18n cleanup sweep** (task #20)~~ — **DONE 2026-05-25**. Finished the Table → Round rename across message *text* in all 5 locales (15 keys × 5 + `judge-table.title` + 2 comment lines). Keys themselves were left unchanged (`error.judging-table.*`, `email.judging-table-ready.*`) to keep code-side BusinessRuleException strings stable. UI terminology now: "Round" for JudgingRound (PT/ES: ronda; IT: round; PL: runda); "Table" reserved for PhysicalTable. Min-judges field label and table-ready emails renamed too. Walkthrough updated (table.md lines 106 / 1599 / 1604 / 1790 / 1954 / 1960 / 2186-2191). 1179 tests passing (unchanged — pure text edits).
...
*Scoring rounds switch to `round.entries` as source of truth (2026-05-24):*
- `ScoresheetService.createScoresheetsForTable` uses `round.entries` when
  non-empty (looking up each entry via `entryService.findEntryById`);
  falls back to the derived `findEntriesByFinalCategoryId` set when empty
  (back-compat). RECEIVED + already-has-scoresheet filters preserved.
- `JudgingService.startRound` auto-populates `round.entries` from the
  derived set when admin didn't explicitly assign any — preserves the
  pre-split-category default while making `round.entries` the source of
  truth from now on. Pre-assigned entries are not overwritten.
- 3 new tests (1 in `ScoresheetServiceTest`, 2 in `JudgingServiceMedalRoundTest`).

*startRound polymorphism on round.type (2026-05-24):*
- `JudgingService.startRound` now branches on `round.type`. SCORING path
  unchanged. MEDAL path skips: min-judges enforcement (medal rounds may
  use fewer judges), category-entry auto-population (medal rounds
  consolidate from scoring rounds — handled in cascade-migration
  cycle), scoresheet creation, CategoryJudgingConfig ensure, and
  `RoundStartedEvent` publication (scoring-only event triggers a
  judge-notification email).
- Shared validations for both types: auth, frozen-results, physical
  table required, physical-table-busy, judge-active-conflict.
- 1 new test in `JudgingServiceMedalRoundTest`.

*Cascade migration to medal JudgingRound (2026-05-24):*
- `ScoresheetServiceImpl.cascadeMarkCategoryReadyIfAllTablesComplete`
  now filters to SCORING rounds when checking "all complete" (medal
  rounds are the destination, not part of the source set).
- When all SCORING rounds in a category COMPLETE, the medal JudgingRound
  (type=MEDAL) for the category is marked READY via `markReady()` —
  the new state of record.
- Legacy `CategoryJudgingConfig.medalRoundStatus` is still updated to
  preserve the existing MedalRoundView / JudgingAdminView Medal Rounds
...
2. **Pick the next cycle.** The remaining caller-migration pieces in
   ...
   *Service-layer changes that don't need UI changes yet:*
   - ✅ **Scoresheet creation switching to `round.entries`** — DONE
     2026-05-24. `createScoresheetsForTable` uses `round.entries` when
     non-empty (fallback to derived when empty). `startRound`
     auto-populates `round.entries` from the derived set when admin
     didn't explicitly assign any. Pre-assigned entries are preserved.
   - ✅ **`startRound` polymorphism on round type** — DONE 2026-05-24.
     `startRound` branches on `round.type`. SCORING path unchanged.
     MEDAL path skips: min-judges enforcement, category-entry
     auto-population, scoresheet creation, CategoryJudgingConfig
     ensure, and RoundStartedEvent publication.
   - ✅ **Cascade marks medal `JudgingRound` READY** — DONE 2026-05-24.
     `cascadeMarkCategoryReadyIfAllTablesComplete` filters to SCORING
     rounds for the "all complete" check; when met, marks any PENDING
     medal `JudgingRound` for the category READY. Legacy
     `CategoryJudgingConfig.medalRoundStatus` still updated as
     transitional bridge (UI not yet migrated).
   - **Auto-fill on SCORE_BASED writes confirmed=false MedalAwards** —
     the cascade currently doesn't run auto-fill (the rank-based
     gold/silver/bronze cascade lives on `startMedalRound`, which
     hasn't been migrated yet). Once the medal-round flow is fully
     migrated to JudgingRound, the SCORE_BASED auto-fill should run
...
3. **Task #4 (UI restructure)** is the next major chunk. Service layer
   ...
   **Cycle 7 — deferred MedalRoundView migration + V22 contraction (2026-05-24):**
   ✅ **DONE** (2 commits: `3a9abd1` migration + `9e08429` contraction).
   - **MedalRoundView reads + writes migrated to medal JudgingRound.**
     `currentStatus()`/`currentMode()`/`currentPhysicalTableId()` now source
     from `medalRound.getStatus()`/etc. Finalize/Reopen/Reset buttons call
     new round-id service methods `completeMedalRoundById`/
     `reopenMedalRoundById`/`resetMedalRoundById`.
   - **Other readers migrated**: `JudgingAdminView.allCategoryRoundsComplete()`,
     `RoundView.medalRoundLocksRevert()`, `ScoresheetServiceImpl` (3 sites:
     setAdvancedToMedalRound guard, revertToDraft guard + retreat,
     deleteScoresheet guard + retreat), `JudgingServiceImpl`
     (`findActiveCategoryConfigsForJudge`, recordMedal/updateMedal/
     deleteMedalAward guards via `requireMedalRoundActive`, BOS startup
     guard). All route through `JudgingService.getEffectiveMedalRoundStatus`.
   - **Legacy methods + fields DELETED**: `JudgingService.startMedalRound/
     completeMedalRound/reopenMedalRound/resetMedalRound/
     assignMedalRoundToPhysicalTable` (5 methods).
     `CategoryJudgingConfig.medalRoundStatus`/`physicalTableId` fields +
     all CJC state-machine domain methods (`markReady`/`markPending`/
     `startMedalRound`/`completeMedalRound`/`reopenMedalRound`/
     `resetMedalRound`/`assignToPhysicalTable`).
     `JudgingServiceImpl.autoPopulateMedalsByScore` helper + its only
     caller (legacy startMedalRound). `MedalRoundStatus` enum entirely.
   - **V22 + V29 in-place edits**: V22 drops the `medal_round_status`
     column. V29 drops the `category_judging_configs.physical_table_id`
     ADD COLUMN. `medal_round_mode` column kept (not renamed to `mode`
     — cosmetic, deferred).
   - **New domain method**: `JudgingRound.resetToReady()` for ACTIVE → READY.
   - **New error key**: `error.medal-round.not-found` in EN/PT/ES/IT/PL.
   - **Test updates**: 4 obsolete tests deleted (3 in
     `JudgingServiceMedalRoundTest`, 1 each in `JudgingAdminViewTest` +
     `JudgingServicePhysicalTableTest`). `MedalRoundViewTest` +
     `MyJudgingViewTest` setups now create the medal JudgingRound
     explicitly (production cascade auto-creates; tests bypass cascade).
     `JudgingServiceMedalsBosTest` helpers rewritten to stub a medal
     `JudgingRound` instead of mutating CJC. `JudgingServiceFreezeGuardTest`
     freeze guards switched to round-id API. **1154 tests passing**
     (was 1158, -4 from obsolete-test deletion).
   - **New gap surfaced**: SCORE_BASED auto-fill was previously on legacy
     `startMedalRound`; now deleted. Once a Start button arrives on
     MedalRoundView/Rounds tab, the auto-fill (writes `confirmed=false`
     MedalAwards) needs to re-emerge — either on `startRound` for MEDAL
     type or in the READY-marking cascade.

**Open gap to address during §12.6 walkthrough:** ~~No "Start" UI for medal
rounds.~~ ✅ **DONE 2026-05-24 (cycle 8, commit `56b00d1`).** MedalRoundView
now has a Start button visible at READY (admin-only, disabled with explanatory
tooltip when no physical table is assigned). SCORE_BASED auto-fill is wired
back into `startRound` for MEDAL type — pre-populates top-3 medals as
`confirmed=false` MedalAwards, stopping the cascade on ties within a slot.

**Cycle 8 work (2026-05-24) — third batch:**
- **Commit `56b00d1`** — Start button on MedalRoundView + SCORE_BASED auto-fill
  in `startRound` for MEDAL type (private helper `autoPopulateMedalsByScore`
  re-introduced, mirrors the version deleted in cycle 7). 5 new i18n keys
  in EN/PT/ES/IT/PL. 1 new test (`shouldStartReadyMedalRoundWhenAdminConfirms`).
  1154 → 1155.
- **Commit `32c201a`** — walkthrough rewrite §12.6 (Rounds tab),
  §12.7 (Results tab), §12.8 (BOS), §12.12 (MedalRoundView gets §12.12.0
  for Open at READY + Start). Stale `ROUND_1` → `ACTIVE`, "Tables grid"
  → "Rounds grid", "Medal Rounds tab" → "Rounds tab → filter Medal"
  fixes elsewhere. Docs only.
- **Commit `cbd6eec`** — Assign Entries dialog on JudgingAdminView Rounds
  tab (📦 button per SCORING row at PENDING; multi-select grid;
  enforces 1:1 with helpful `error.entry.already-on-round` error)
  + Assign Judges button on MedalRoundView (visible at PENDING/READY;
  same dialog pattern as scoring rounds). 11 new i18n keys.
- **Commit `047abe3`** — DevDataInitializer.seedProfissionalForJudging
  now pre-creates 2 split scoring rounds for Profissional M1A (Panel A:
  judges 1+2, Table 1, 2 entries; Panel B: judges 4+5, Table 2,
  3 entries) and a PENDING medal round for M1B with independent judges
  (1+2+6, Table 4). Walkthrough §12.6.7.1 + §12.12.0.1 walk through
  the seeded scenarios.

**Cycle 9 (2026-05-24) — both walkthrough gaps closed:**
- **Gap 1 (mode + PT picker on MedalRoundView header) — DONE.** At
  PENDING/READY the header now renders editable Select<MedalRoundMode> +
  Select<PhysicalTable> next to a status badge; once the round starts the
  Selects vanish and the read-only mode/PT lines come back. Backed by
  new `JudgingRound.updateMedalMode(mode)` (gated PENDING/READY +
  MEDAL-type only) + new `JudgingService.updateMedalRoundMode` + a
  relaxed `assignRoundToPhysicalTable` gate (PENDING ∨ READY, was
  PENDING-only — was blocking PT-assignment on cascade-auto-created
  READY medal rounds). New EN/PT/ES/IT/PL keys:
  `medal-round.physical-table.{none-defined,updated}`,
  `medal-round.mode.{comparative,score-based,updated}`,
  `error.medal-round.mode-{locked-after-start,not-applicable}`. 9 new
  tests (4 entity + 5 service + 3 view).
- **Gap 2 (medal-round judge reassignment past READY) — DONE.**
  `MedalRoundView` Assign Judges button now enabled through
  PENDING/READY/ACTIVE (only locked at COMPLETE). Service-side
  `JudgingServiceImpl.removeJudge` skips the min-judges check for MEDAL
  type — scoring's `minJudgesPerRound` doesn't apply to medal panels.
  Past `MedalAward.awardedBy` rows are preserved on judge removal. 2
  new tests (1 service + 1 view). Walkthrough §12.12.0.2 updated.

**1169 tests passing** (+14 since cycle 8).
...
- #3 Entity + service refactor + contraction phase — **COMPLETE** (V22 contraction in cycle 7)
- #4 UI restructure (Rounds + Results + BOS tabs) — **COMPLETE** (cycles 6a-c)
- #5 Dev seed updates — **COMPLETE** (cycle 8 — split-category M1A + medal-round judges)
- #6 Walkthrough rewrite §12.6-§12.8 — **COMPLETE** (cycle 8) + §12.6.7.1 + §12.12.0/0.1 added for new UIs
- #7 i18n (5 locales) — done as we go (cycle 7 added 2 keys; cycle 8 added 16 keys for Start dialog + Assign Entries dialog + Assign Judges medal button + error.entry.already-on-round)
- #8 Resume walkthrough + ship v0.4.0 — **READY** to run the full walkthrough end-to-end
...
- **Judging-workflow audit fixes** — Completed 2026-05-22 on `feature/judging-module`. A domain-aware audit of the judging workflow (grounded in `docs/reference/chip-competition-rules.md`) found four gaps, all fixed: **(A)** only **RECEIVED** entries are judged — `ScoresheetServiceImpl.createScoresheetsForTable` and `JudgingService.findMedalRoundEntries` now skip non-RECEIVED entries, so a withdrawn or never-arrived bottle no longer gets a phantom scoresheet. **(B)** `JudgingAdminView` shows a red warning ("{N} entries have no judging category…") via new `EntryService.countEntriesNeedingFinalCategory` — a paid entry can't be silently left unjudged. **(C)** `ScoresheetView`'s entry card now shows the declared attributes a judge needs to judge to style (category, sweetness, carbonation, ABV, honey, other ingredients, wood ageing, notes) — judges work from poured coded samples, not the labelled bottle, so the on-screen detail is essential; reuses the existing `entries.view.*` i18n keys. **(D)** new read-only **`StewardView`** (`/my-stewarding`) — tables + assigned judges + entries for every JUDGING-or-later division the user stewards; adds `CompetitionService.findCompetitionsBySteward`, `StewardChecker` interface (root) + `StewardCheckerImpl` (competition.internal), a MainLayout sidebar entry gated on steward status, and `JudgingService.findJudgeUserIdsForTable` (loads the lazy assignment collection in-transaction). New `steward.*` i18n keys in all five languages. Walkthrough §12.5/§12.6.4/§12.11 updated + new §12.16 StewardView section. 9 new tests; 1098 total. No migration.
...
- **Categories tab grid + Add Category parent select — REGISTRATION scope only** — Completed 2026-05-18 (on main, merged into feature/judging-module 2026-05-19). After Initialize Judging Categories (which clones REGISTRATION → JUDGING with the same codes), the DivisionDetailView's Categories tab TreeGrid showed duplicate codes (M1A, M1B, …) because `refreshCategoriesGrid()` sourced from `findDivisionCategories()` which returns all scopes. The same bug also surfaced in the Add Category dialog's Custom tab → Parent Category Select (let admins pick a JUDGING-scope parent for a new REGISTRATION-scope category). Caught during v0.3.0 mid-walkthrough sanity-test F. Fix: both call sites now use `CompetitionService.findRegistrationCategories(divisionId)` (introduced in `4cdd18e`). Third instance of the same scope-filter pattern after entry primary-category dropdowns. 2 new UI tests (`shouldExcludeJudgingScopeCategoriesFromCategoriesTabGrid`, `shouldExcludeJudgingScopeCategoriesFromCustomCategoryParentSelect`).
- **Final Category picker — leaf categories only** — Completed 2026-05-17 (on main, merged into feature/judging-module 2026-05-19). The Final Category Select in the admin Edit Entry dialog used to list ALL judging-scope categories — both parents (M1, M2, ...) and subcategories (M1A, M1B, ...) — letting an admin assign a parent that has subcategories, which doesn't make semantic sense for a "final" classification. Fix: new `CompetitionService.findLeafJudgingCategories(divisionId)` returns only categories whose id is NOT referenced as parentId by any other JUDGING-scope category in the same division. Standalone categories without children are still considered leaves (e.g. a custom "CX1 — Combined" without subcategories shows). Service-side validation in `assignFinalCategory` is unchanged (still allows any JUDGING-scope id — UI is the only gate); a future hardening could reject non-leaf assignments service-side. 1 new unit test.
- **Primary Category dropdowns — no duplicates after judging-category init** — Completed 2026-05-17 (on main, merged into feature/judging-module 2026-05-19). Once judging categories were initialized (cloned from registration with same codes), the entry primary-category dropdowns in the admin Add Entry, admin Edit Entry, and entrant edit dialogs all listed BOTH the REGISTRATION and JUDGING-scope rows (loaded via `findDivisionCategories` which doesn't filter by scope), so M1A, M1B, etc. appeared twice. Fix: new `CompetitionService.findRegistrationCategories(divisionId)` (parallel to `findJudgingCategories`); all 3 dropdown sites now use it. `findDivisionCategories` is still called once at view init to cache all-scope rows for by-ID lookups (e.g., the grid Final Category column needs to resolve JUDGING-scope IDs). 1 new unit test `shouldFindRegistrationCategoriesOnlyExcludingJudgingScope`.
- **Final Category picker — no fallback to registration categories** — Completed 2026-05-17 (on main, merged into feature/judging-module 2026-05-19). Reversed the original design decision that let the admin entry-edit dialog fall back to ALL division categories when no JUDGING-scope categories existed. Problem: an entry's `finalCategoryId` could end up pointing to a registration-scope row, which `EntryJudgingCategoryDeletionGuard` doesn't protect (it only walks JUDGING-scope), so a later admin could silently break the entry by removing the registration category. Fix: `DivisionEntryAdminView` now disables the Final Category Select with helper text "Initialize judging categories first to assign a final category" when `judgingCategories.isEmpty()`; `EntryService.assignFinalCategory()` now throws `BusinessRuleException("error.entry.final-category-no-judging-categories")` when a non-null `finalCategoryId` is passed before judging categories are initialized. Clearing (null) is still allowed unconditionally, so any stale prior assignments can be cleaned up. Also added the previously-missing `error.entry.final-category-not-judging` translation. 1 new test + 1 reversed test.
- **MFA verify Enter shortcut fix** — Completed 2026-05-17 (on main, merged into feature/judging-module 2026-05-19). `MfaVerifyView` code field now uses `ValueChangeMode.EAGER` so the Enter shortcut listener sees the currently typed value instead of the stale on-blur value. Before: user had to Tab/click out of the field before pressing Enter to submit. Same pattern as `LoginView` (password), `SetPasswordView` (password+confirm), `UserListView` (filter). 1 new UI test in `MfaVerifyViewTest` asserts `getValueChangeMode() == EAGER`.
- **i18n cleanup: dialog buttons** — Completed 2026-05-16 (on main, merged into feature/judging-module). Replaced 40+ hardcoded "Cancel"/"Save"/"Close"/"Delete" button strings with `getTranslation("button.*")` across `UserListView`, `DivisionEntryAdminView`, `DivisionDetailView`, `CompetitionListView`, `CompetitionDetailView`. Added missing `button.save` and `button.close` keys in EN+PT (`button.cancel` and `button.delete` already existed). Tests still pass because they run in default ENGLISH locale where `getTranslation("button.save") == "Save"`.
- **i18n gaps in sidebar + entry edit dialog** — Completed 2026-05-16 (on main, merged into feature/judging-module). `MainLayout` now resolves sidebar nav items via `getTranslation()` for SYSTEM_ADMIN ("Competitions"/"Users") and competition-admin ("My Competitions") roles — they were hardcoded English with a misleading "Admin-only, no i18n" comment. The Final Category select label and "— Not assigned —" placeholder in the admin entry edit dialog (`DivisionEntryAdminView`) were also hardcoded. New keys: `nav.users`, `entry-admin.entries.edit.final-category`, `entry-admin.entries.edit.final-category.unset` (EN+PT).
- **MFA email reset (Lost your device?)** — Completed 2026-05-16 (on main, merged into feature/judging-module). Recovery path for SYSTEM_ADMIN locked out of TOTP. On `/mfa`, a "Lost your device?" button triggers `EmailService.sendMfaReset(email, locale)`. Reset link points to new `/mfa-reset` route; clicking validates the JWT, calls `UserService.completeMfaReset(token)` which clears `totpSecret` + `mfaEnabled`, invalidates session, then renders a confirmation page with a "Continue to Login" button. Token validity is 1 hour (short — security-critical, not the 7-day generic). Rate-limited via the existing per-user 5-minute cooldown (`mfa-reset` key). i18n keys added in EN+PT (`mfa.verify.lost-device`, `mfa.verify.reset-link-sent`, `mfa.reset.*`, `email.mfa-reset.*`). New: `JwtMagicLinkService.generateMfaResetLink`, `UserService.completeMfaReset`, `EmailService.sendMfaReset`, `SmtpEmailService` impl with `MFA_RESET_TOKEN_VALIDITY=1h`, `MfaResetView` (`@AnonymousAllowed`), `MfaVerifyView` updated. SecurityConfig adds `/mfa-reset` to permitAll. Visibility fix: `MfaVerifyView` and `MfaResetView` made public so Karibu's `autoDiscoverViews` can find them (it skips package-private classes). 12 new tests (1 JWT + 3 UserService + 3 SmtpEmailService + 3 MfaResetView + 2 MfaVerifyView).
- **Codebase-wide inline FQN cleanup** — Completed 2026-05-16 (on main, merged into feature/judging-module). Applied the new CLAUDE.md "no inline fully-qualified names" rule across 10 files: `LabelPdfService`, `CompetitionService`, `CompetitionDetailView`, `DivisionDetailView`, `DivisionEntryAdminView`, `MyEntriesView`, `UserServiceTest`, `CompetitionServiceTest`, `DivisionDetailViewTest`, `CompetitionDetailViewTest`. Added the missing imports (e.g. `BufferedImage`, `Sort`, `HierarchicalQuery`, `ValueChangeMode`, `SortDirection`, `Image`, `IntegerField`, `Component`, `Function`, `DateTimeFormatter`/`FormatStyle`, `Collectors`) and replaced inline FQNs with simple names. Kept the documented exceptions intact (`org.springframework.security.core.userdetails.User` collision; `@org.springframework.modulith.ApplicationModule` in package-info.java). No behavior change. NOTE: FJM-specific code added on this branch (judging + awards modules) may still have inline FQN violations — flagged for a future cleanup sweep on the branch.
- **Judging-category parent-delete guard fix** — Completed 2026-05-16 (on main, merged into feature/judging-module). `CompetitionService.removeJudgingCategory()` now walks the requested category + all descendants recursively, runs the `JudgingCategoryDeletionGuard` on every node, then deletes bottom-up so parent_id FKs are satisfied. Previously the guard was only checked on the requested category, so deleting a parent whose child was referenced by `entry.final_category_id` triggered a raw `DataIntegrityViolationException` instead of the friendly `error.category.judging-has-entries` message — the view caught only `BusinessRuleException`, so the user saw nothing. 1 new integration test in EntryModuleTest. Also: project-wide convention added to CLAUDE.md banning inline fully-qualified class/method names (with documented exceptions for the User collision and `package-info.java` annotations).
- **Webhook post-registration guard** — Completed 2026-05-16 (on main, merged into feature/judging-module). `WebhookService.processOrderPaid()` now checks `division.getStatus().allowsRegistrationActions()` after the mutual-exclusivity and role-conflict checks. Orders for a division past REGISTRATION_OPEN are marked NEEDS_REVIEW with reason "Registration closed: division no longer accepting new credits" — no credits awarded, no ENTRANT promotion, no misleading credit-notification email. Admin gets the existing OrderRequiresReviewEvent alert email. 1 new unit test in WebhookServiceTest.
- **Credits-grid stale-refresh fix** — Completed 2026-05-16 (on main, merged into feature/judging-module). DivisionEntryAdminView's Credits tab now auto-refreshes after entry mutations that change per-user entry count (admin create, delete, withdraw, revert). Previously only `refreshEntriesGrid()` was called, leaving the Credits grid's per-user `Entries` column stale until manual reload. Symmetric with the existing pattern where credit mutations refresh both credits grid and balance. Regression check is manual (Karibu dialog disambiguation cost > value for this UI bookkeeping bug).
