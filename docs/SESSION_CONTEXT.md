...
### CURRENT (2026-05-26): Resume the walkthrough mid-§12.6.8 → §12.6.8.1 (Option A landed)

The walkthrough paused mid-§12.6.8 when the user surfaced that a SCORE_BASED
medal round couldn't have entries assigned to it without a preceding scoring
round — the eligible-entries dialog filter required SUBMITTED scoresheets.
That gap blocked the "small category, run scoring at the medal round itself"
use case the user described as **MEDAL — SCORING**. Option A landed today:
the medal round now owns its scoresheets in that mode. See "Recently
completed" below for the full 7-cycle summary.

**State on disk** (`feature/judging-module`, 1217 tests passing — verify with
`mvn test -Dsurefire.useFile=false`). Top commit is whatever the user committed
to land Option A.
