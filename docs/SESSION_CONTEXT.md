...
**Tests:** 1104 passing (`mvn test -Dsurefire.useFile=false`) — verified 2026-05-23 after the `DivisionAdvanceGuard` family, `AwardsService.republish` refactor, BOS-places lock moving from REGISTRATION_CLOSED to JUDGING, the deadline-stale-value Settings-tab bug fix, and the removal of the per-competition comment-languages restriction (judges now pick from all ISO 639-1 codes).
