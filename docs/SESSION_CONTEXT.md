...
**Tests:** 1112 passing (`mvn test -Dsurefire.useFile=false`) — verified 2026-05-23 after the `DivisionAdvanceGuard` family (now including `EntryFinalCategoryAdvanceGuard`), `AwardsService.republish` refactor, BOS-places lock moving from REGISTRATION_CLOSED to JUDGING, deadline-stale-value Settings-tab bug fix, removal of the per-competition comment-languages restriction, dev seed beef-up (Amadora 10 entries with status variety + Profissional 20 entries pre-staged at JUDGING), `EntryService.assignFinalCategoriesByCode` bulk auto-assign button, `EntryReceivedEvent` + `EntryReceivedScoresheetListener` syncing scoresheets on SUBMITTED → RECEIVED during JUDGING, and `EntryStatusRevertGuard` interface + `JudgingScoresheetEntryRevertGuard` impl blocking RECEIVED revert / withdraw while a scoresheet exists (admin must use the new TableView "Delete scoresheet" action first).
