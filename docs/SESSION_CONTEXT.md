...
**▶ LATEST (2026-06-05 — entrant "View scoresheet" dialog redesign, part 1; tests green; NOT yet committed — working tree dirty).**
**⚑ NEXT:** verify the dialog on the Fast Track / Mostra GOLD entry (`entrant@example.com` → My Results → 👁), then continue the entrant-scoresheet redesign on the **PDF** side (still reads "Anonymized Scoresheet"? — actually the entrant PDF already says "Scoresheet" + no judge row per §13.5; remaining PDF wants: medal/BOS/advanced surfaced + confirm entry number not code). User feedback this turn was about the **on-screen dialog** only.
