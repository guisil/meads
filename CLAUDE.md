...
├── JudgeAssignmentChecker.java          ← Interface for sidebar gating of "My Judging" (impl in judging.internal)
├── StewardChecker.java                  ← Interface for sidebar gating of "My Stewarding" (impl in competition.internal)
...
├── CompetitionService.java              ← Application service (public API). `advanceDivisionStatus` runs `DivisionAdvanceGuard`s; `revertDivisionStatus` runs `DivisionRevertGuard`s; `publishDivision` (awards-only) bypasses advance guards to do the DELIBERATION → RESULTS_PUBLISHED transition that's now blocked from the manual UI path.
├── DivisionAdvanceGuard.java            ← Guard interface for blocking premature status advances
...
└── internal/                            ← Module-private
    ...
    ├── RegistrationCategoryAdvanceGuard.java ← DivisionAdvanceGuard impl (blocks DRAFT → REGISTRATION_OPEN with no registration categories)
    ├── JudgingCategoryAdvanceGuard.java     ← DivisionAdvanceGuard impl (blocks REGISTRATION_CLOSED → JUDGING when judging categories not initialized)
...
└── internal/                                ← Module-private
    ...
    ├── AwardsServiceImpl.java               ← Service impl (@Validated, @Transactional). `publish` requires DELIBERATION + uses `CompetitionService.publishDivision`; `republish` also requires DELIBERATION (changed from RESULTS_PUBLISHED) and advances status itself via the same method.
    ├── BlockManualPublishAdvanceGuard.java  ← DivisionAdvanceGuard impl (blocks manual DELIBERATION → RESULTS_PUBLISHED so admins must go through the awards Publish/Re-publish dialogs)
    ├── AwardsAdminView.java                 ← Admin view (@PermitAll + beforeEnter auth) — at DELIBERATION shows Publish or Re-publish depending on prior publication history; at RESULTS_PUBLISHED shows Announce + Revert (no in-place Re-publish — must revert first)
...
| `judging` | **Exists** | 7 aggregates (Judging, JudgingRound+JudgeAssignment, CategoryJudgingConfig, Scoresheet+ScoreField, MedalAward, BosPlacement, JudgeProfile), services (JudgingService, ScoresheetService, JudgeProfileService, CoiCheckService), 13 events, `JudgingNotificationListener` (emails judges on `RoundStartedEvent` / `ScoresheetRevertedEvent` / `MedalRoundActivatedEvent`; the other 10 events are admin-triggered state transitions with no judge-facing CTA, deliberately not consumed), 3 cross-module guards (JudgingDivisionStatusRevertGuard, JudgingMinJudgesLockGuard, JudgingCompleteAdvanceGuard — blocks JUDGING → DELIBERATION until `Judging.phase = COMPLETE`), `JudgeAssignmentChecker` interface (root) for sidebar gating. Views: JudgingAdminView (Tables/Medal Rounds/BOS — Medal Rounds tab has a per-category mode selector + "Open medal round" link), RoundView (admin scoresheet management + row click → ScoresheetView), MyJudgingView (judge hub), ScoresheetView (judge form: 5 score fields + comments + comment language + advance + submit; read-only when SUBMITTED), MedalRoundView (shared judge/admin medal-awarding form: per-entry Gold/Silver/Bronze + Withhold/Clear, live summary, admin Finalize/Reopen/Reset; SCORE_BASED mode shows a tied-slot banner + highlights tied rows via `recomputeScorePreview`), BosView (admin placement form with Assign/Reassign/Delete; read-only when COMPLETE), StewardView (`/my-stewarding`, read-only — tables + judges + entries for every division the user stewards). Only RECEIVED entries are judged (`createScoresheetsForTable` skips SUBMITTED/WITHDRAWN); JudgingAdminView warns when entries still lack a final category. `JudgingErrorKeyCoverageTest` guards EN+PT translation coverage for every `BusinessRuleException` key the module throws. Reference: `docs/reference/chip-competition-rules.md` |
| `awards` | **Exists** | Publication lifecycle on top of judging data: `publish` / `republish` / `sendAnnouncement`. Single `Publication` audit-only aggregate; judging data frozen-in-place via `DivisionStatus.isResultsFrozen()` guard on every judging mutator. `BlockManualPublishAdvanceGuard` forces admins through the Publish/Re-publish dialogs so every RESULTS_PUBLISHED transition creates a Publication audit row. `publish` and `republish` both require status = DELIBERATION; `republish` advances status itself (revert → edit → republish; no manual re-advance hack). Views: `AwardsAdminView` (publish/republish/announce/revert), `AwardsPublicResultsView` (anonymized, `@AnonymousAllowed`), `MyResultsView` + `MyScoresheetView` (entrant-facing drill-in). `ScoresheetPdfService` in judging module supports ANONYMIZED + FULL modes. Reference: `docs/reference/chip-competition-rules.md`, design+plan `docs/plans/2026-05-12-awards-module-{design,plan}.md` |
