...
| `judging` | **In progress** (Phase 6 views) | 7 aggregates (Judging, JudgingTable+JudgeAssignment, CategoryJudgingConfig, Scoresheet+ScoreField, MedalAward, BosPlacement, JudgeProfile), services (JudgingService, ScoresheetService, JudgeProfileService, CoiCheckService), 13 events, 2 cross-module guards (JudgingDivisionStatusRevertGuard, JudgingMinJudgesLockGuard). Views: JudgingAdminView (Tables/Medal Rounds/BOS tabs complete), TableView (admin scoresheet management complete; row → ScoresheetView pending). Reference: `docs/reference/chip-competition-rules.md` |
